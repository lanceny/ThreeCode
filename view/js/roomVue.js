  ...
  methods: {
    ...
    sendMessage: function (flag) {
      ...
      if (flag == Aspiration) {
        ...
        // axiosにパスを投げる
        axios.post("/sendMessageAspiration", params).then((response) => {
        ...
        axios.post("/sendMessageLookback", params).then((response) => {
          if (response.status != 200) {
            throw new Error("レスポンスエラー");
          } else {
    ...
    fetchAllMessageAspiration() {
      ...
      axios
        .get("/fetchallasp/" + location.pathname.replace("/room/", ""))
        .then((response) => {
          if (response.status != 200) {
            throw new Error("レスポンスエラー");
          } else {
    ...
    fetchAllMessageLookback() {
      axios
        .get("/fetchalllkb/" + location.pathname.replace("/room/", ""))
        .then((response) => {
          if (response.status != 200) {
            throw new Error("レスポンスエラー");
          } else {
    ...
    doDeleteMessage(message) {
      ...
      if (this.delflag) {
        ...
        axios
          .post(
            "/deleteMessage/" + location.pathname.replace("/room/", ""),
            params
          )
          .then((response) => {
            if (response.status != 200) {
